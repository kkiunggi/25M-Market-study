...
const defs=[
{id:"us2y",label:"US 2Y",note:"Fed 정책 기대",unit:"%",loader:()=>fredCsv("DGS2"),source:"FRED"},
{id:"us10y",label:"US 10Y",note:"성장·인플레·term premium",unit:"%",loader:()=>fredCsv("DGS10"),source:"FRED"},
{id:"kr3y",label:"국고채 3Y",note:"BOK 기대",unit:"%",loader:()=>ecos("010200000"),source:"BOK ECOS"},
{id:"kr10y",label:"국고채 10Y",note:"한국 장기금리",unit:"%",loader:()=>ecos("010210000"),source:"BOK ECOS"},
{id:"usdkrw",label:"USD/KRW",note:"원화",unit:"",loader:frankfurter,source:"Frankfurter"},
{id:"usd",label:"USD Broad",note:"DXY 대체: broad dollar index",unit:"",loader:()=>fredCsv("DTWEXBGS"),source:"FRED"},
{id:"sp500",label:"S&P 500",note:"Risk sentiment",unit:"",loader:()=>fredCsv("SP500"),source:"FRED"},
{id:"nasdaq",label:"Nasdaq",note:"Growth / risk",unit:"",loader:()=>fredCsv("NASDAQCOM"),source:"FRED"},
{id:"brent",label:"Brent",note:"인플레·지정학",unit:"$",loader:()=>fredCsv("DCOILBRENTEU"),source:"FRED"},
{id:"vix",label:"VIX",note:"위험회피",unit:"",loader:()=>fredCsv("VIXCLS"),source:"FRED"}
];
let cache={t:0,data:null};
app.get("/api/markets",async(req,res)=>{
 if(cache.data&&Date.now()-cache.t<300000)return res.json(cache.data);
 const results=await Promise.all(defs.map(async d=>{try{return {...d,...await d.loader(),source:d.source,ok:true}}catch(e){return {...d,ok:false,error:e.message,series:[]}}}));
 const payload={updatedAt:new Date().toISOString(),items:results}; cache={t:Date.now(),data:payload}; res.json(payload);
});
function cleanQuery(q){return String(q||"").replace(/[^\p{L}\p{N}\s"()\/\-.]/gu," ").slice(0,400)}
app.get("/api/news",async(req,res)=>{
 const asset=cleanQuery(req.query.asset||"");
 const base=asset||'("Federal Reserve" OR Treasury OR inflation OR "Bank of Korea" OR "Korean won" OR "S&P 500" OR Nasdaq OR "Brent crude")';
 const u=`https://api.gdeltproject.org/api/v2/doc/doc?query=${encodeURIComponent(base)}&mode=ArtList&maxrecords=25&format=json&sort=HybridRel`;
 try{const r=await fetch(u,{headers:{"User-Agent":"25M-Markets/1.0"}});if(!r.ok)throw new Error(`GDELT ${r.status}`);const j=await r.json();res.json({ok:true,articles:(j.articles||[]).map(a=>({title:a.title||"(제목 없음)",source:a.domain||a.sourcecountry||"News",date:a.seendate||"",url:a.url||"",language:a.language||""}))})}catch(e){res.status(502).json({ok:false,error:e.message,articles:[]})}
});
app.post("/api/feedback",async(req,res)=>{
 const key=process.env.OPENAI_API_KEY;if(!key)return res.status(503).json({ok:false,error:"OPENAI_API_KEY is not configured on the server."});
 ...
 try{const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${key}`},body:JSON.stringify({model:process.env.OPENAI_MODEL||"gpt-5.4",input:prompt})});const j=await r.json();if(!r.ok)throw new Error(j.error?.message||`OpenAI ${r.status}`);let text=j.output_text;if(!text&&Array.isArray(j.output))text=j.output.flatMap(o=>o.content||[]).map(c=>c.text||"").join("\n");res.json({ok:true,text:text||"응답을 읽지 못했습니다."})}catch(e){res.status(502).json({ok:false,error:e.message})}
});
